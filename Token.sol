...
contract Token is TokenUtils{

     constructor(string memory _name,string memory _symbol,
                address _systemAddress,address payable _tokenHolderWallet,
                address _whiteListAddress,
                uint256 reserveSupply,uint256 holdBackSupply) public TokenUtils(_name,_symbol,_systemAddress,_tokenHolderWallet,_whiteListAddress){
    ...
    */
    ...
    function swapForTokens(uint256 _tokenPrice,address _to,uint256 _value) public returns(bool){
        require(msg.sender == etnTokenAddress || msg.sender == stockTokenAddress ,ERR_ACTION_NOT_ALLOWED);
    ...
    function swapToken(address swapble,uint256 _value) public notZeroValue(_value) notZeroAddress(swapble) returns (bool){
        require(isDirectSwap && (swapble == etnTokenAddress || swapble == stockTokenAddress));
